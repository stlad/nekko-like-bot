...
public class MyCatsInfoReceiveCommand implements WorkflowCommand {
    ...
    public void execute(UpdateData data, TelegramMessageSender sender) {
        ...
        sender.send(MessageBuilder.catPhoto(chatId, flow.getCatInfoDto().getPhoto()));
        sender.send(MessageBuilder.catInfoMenu(chatId, flow.getCatInfoDto().getAuthorTelegramUsername(),
                flow.getCatInfoDto().getCatName(), flow.getCatInfoDto().getCatId()));
    ...
    public WorkflowStep getInitStep() {
        return WorkflowStep.MY_CATS_WAIT_CAT_INFO;
    }
}
