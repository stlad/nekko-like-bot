...
public class MyCatsPrevCommand implements WorkflowCommand {
    ...
    public void execute(UpdateData data, TelegramMessageSender sender) {
        ...
        log.info("Пользователь {} запросил предыущую страницу своих котиков", chatId);
        var flow = workflowRepository.findByChatId(chatId).orElseThrow(() -> new WorkflowNotFoundException(chatId));

        flow.getMyCatsDto().prevPage();
    ...
    public WorkflowStep getInitStep() {
        return WorkflowStep.MY_CATS_VIEW_PREV_PAGE;
    }
}
